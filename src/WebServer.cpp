...
async function get(name,url){if(cache[name])return cache[name];let r=await fetch(url);if(!r.ok){if(name==='news'){status.textContent='News unavailable · retrying automatically';return cache[name]={data:{},alerts:[]}}throw Error('HTTP '+r.status)}return cache[name]=await r.json()}
...
setInterval(()=>{if(activeTab==='deeper-analysis'){cache={};openTab(activeTab,false)}},1000);
refreshTimer=setInterval(()=>{updateMarketStatus();if(!updatesPaused&&!refreshInFlight&&activeTab!=='config'&&activeTab!=='deeper-analysis'){cache={};refreshInFlight=true;openTab(activeTab,false).finally(()=>refreshInFlight=false)}},5000);
    ...
    if (source.contains("data") && source["data"].is_object()) {
        for (auto it = source["data"].begin(); it != source["data"].end(); ++it)
                if (keys.count(it.key())) {
                    ...
                    const NewsDecision decision = decideNews(output["data"][it.key()]);
...
std::string metrics(const std::vector<Position>& holdings, int newsArticles,
    ...
    int holdCount = 0;
    for (const auto& holding : holdings) {
        marketValue += holding.lastPrice * holding.quantity * holding.multiplier;
...
std::string holdingsForUi(const std::string& body,
    ...
    if (data == payload.end() || !data->is_array()) return body;
...
std::string WebServer::stockAnalysis(const std::string& symbol) const {
    ...
    const auto root = projectRoot();
    const std::string inputPath = "/tmp/myfolio_stock_" +
                                  std::to_string(static_cast<long long>(getpid())) + ".csv";
    const std::string outputPath = "/tmp/myfolio_stock_" +
...
std::string WebServer::runDeeperAnalysis() const {
    ...
    const std::string outputPath = "/tmp/myfolio_nlp_" +
                                   std::to_string(static_cast<long long>(getpid())) + ".csv";
    const std::string scriptPath = projectPythonScript();
    const auto root = projectRoot();
    const auto holdingsFile = (root / "config" / "holding.csv").string();
    ...
    while (true) {
        ...
        try {
            if (path == "/login") {
                ...
                html.replace(html.find(marker), marker.size(), std::string(dashboardEnhancements()) + dashboardRefreshGuard() + dashboardVersion() + releaseNotice() + categoryEnhancements() + marker);
                ...
                if (path == "/api/holdings") body = current->holdings;
                else if (path == "/api/positions") body = current->positions;
                else if (path == "/api/news") body = current->news;
            ...
            } else if (path == "/api/config") body = "{\"status\":\"ready\",\"holdings_file\":\"" + holdingsFile_ + "\"}";
            else body = json({{"error", "not found"}}).dump();
        } catch (const std::exception& e) {
            body = json({{"error", e.what()}}).dump();
        }
