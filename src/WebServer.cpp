    ...
    while (true) {
        ...
        try {
            if (path == "/login") {
                ...
                html.replace(html.find(marker), marker.size(), std::string(dashboardVersion()) + releaseNotice() + sortingReleaseNotice() + categoryEnhancements() + requestedDashboardTabs() + requestedDashboardRouting() + requestedDashboardReleaseNotice() + responsiveDashboardNavigation() + marker);
                ...
                const auto global = client_.getNews("global");
                body = global.ok ? global.rawBody : json({{"status", "error"}, {"data", json::array()}, {"error", global.error}, {"source", "unavailable"}}).dump();
